...
def create_actor(session:SessionDep, actor:ActorCreate):
...
def read_actors(session:SessionDep, offset: int = 0, limit: int = Query(default=20, le=20)):
    actors = session.exec(select(Actor).offset(offset).limit(limit)).all()
    return actors

@router.get("/{actor_id}", response_model=ActorPublic)
def read_actor(session:SessionDep, actor_id: uuid.UUID):
...
def update_actor(session: SessionDep, actor_id: uuid.UUID, actor: ActorUpdate):
...
def delete_actor(session: SessionDep, actor_id: uuid.UUID):
    actor = session.get(Actor, actor_id)
    if not actor:
        raise HTTPException(status_code=404, detail="Actor Not Found")
